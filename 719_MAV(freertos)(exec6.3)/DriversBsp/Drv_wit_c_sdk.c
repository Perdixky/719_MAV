...
static void CopeWitData(uint8_t ucIndex, uint16_t *p_data, uint32_t uiLen)
{
    ...
    if(uiReg1Len)
	{
		memcpy(&sReg_s[uiReg1], p_usReg1Val, uiReg1Len<<1);
		p_WitRegUpdateCbFunc(uiReg1, uiReg1Len);
	}
    if(uiReg2Len)
	{
		memcpy(&sReg_s[uiReg2], p_usReg2Val, uiReg2Len<<1);
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_MODBUS:
            if(s_uiWitDataCnt > 2)
            {
                ...
                for(i = 0; i < usTemp; i++)
                {
                    sReg_s[i+s_uiReadRegIndex] = ((uint16_t)s_ucWitDataBuff[(i<<1)+3] << 8) | s_ucWitDataBuff[(i<<1)+4];
                }
...
//		Classis_Interface_in(NULL);
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            if(p_WitI2cReadFunc(s_ucAddr << 1, uiReg, s_ucWitDataBuff, usTemp) == 1)
            {
                ...
                {
                    sReg_s[i+uiReg] = ((uint16_t)s_ucWitDataBuff[(i<<1)+1] << 8) | s_ucWitDataBuff[i<<1];
                }
    ...
    s_uiReadRegIndex = uiReg;
		...
		Classis_Interface_out("f", sReg_s);							//底盘容器写入函数，写入寄存器sReg_s
